...
import coding.lagom.api.{CodingLagomService, GreetingMessage}
import akka.Done
import akka.NotUsed
...
/**
  * Implementation of the CodinglagomService.
  */
class CodingLagomServiceImpl(
  clusterSharding: ClusterSharding,
  persistentEntityRegistry: PersistentEntityRegistry
)(implicit ec: ExecutionContext)
  extends CodingLagomService {

  /**
    * Looks up the entity for the given ID.
    */
  private def entityRef(id: String): EntityRef[CodinglagomCommand] =
    clusterSharding.entityRefFor(CodinglagomState.typeKey, id)
  ...
  override def greetingsTopic(): Topic[api.GreetingMessageChanged] =
    TopicProducer.singleStreamWithOffset { fromOffset =>
      persistentEntityRegistry
        .eventStream(CodinglagomEvent.Tag, fromOffset)
        .map(ev => (convertEvent(ev), ev.offset))
    }

  private def convertEvent(
    helloEvent: EventStreamElement[CodinglagomEvent]
