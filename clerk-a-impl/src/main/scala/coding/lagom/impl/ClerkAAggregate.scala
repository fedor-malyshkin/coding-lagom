...
/**
  * This provides an event sourced behavior. It has a state, [[ClerkAState]], which
  ...
  */
object ClerkABehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */
  def create(entityContext: EntityContext[ClerkACommand]): Behavior[ClerkACommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, ClerkAEvent.Tag)
      )
...
case class ClerkAState(message: String, timestamp: String) {
  ...
  def applyEvent(evt: ClerkAEvent): ClerkAState =
    evt match {
...
object ClerkAState {
  ...
  /**
    ...
    */
  def initial: ClerkAState = ClerkAState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  implicit val format: Format[ClerkAState] = Json.format
}

/**
  * This interface defines all the events that the ClerkAAggregate supports.
  */
sealed trait ClerkAEvent extends AggregateEvent[ClerkAEvent] {
  def aggregateTag: AggregateEventTag[ClerkAEvent] = ClerkAEvent.Tag
}

object ClerkAEvent {
  val Tag: AggregateEventTag[ClerkAEvent] = AggregateEventTag[ClerkAEvent]()
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends ClerkAEvent
...
/**
  ...
  */
trait ClerkACommandSerializable
...
/**
  ...
  * application loader.
  */
object ClerkASerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[ClerkAState],
