...
/**
  * This provides an event sourced behavior. It has a state, [[CodinglagomState]], which
  ...
  */
object CodingLagomBehavior {

  /**
    * Given a sharding [[EntityContext]] this function produces an Akka [[Behavior]] for the aggregate.
    */ 
  def create(entityContext: EntityContext[CodinglagomCommand]): Behavior[CodinglagomCommand] = {
    ...
    create(persistenceId)
      .withTagger(
        ...
        AkkaTaggerAdapter.fromLagom(entityContext, CodinglagomEvent.Tag)
      )
  ...
  private[impl] def create(persistenceId: PersistenceId) = EventSourcedBehavior
      .withEnforcedReplies[CodinglagomCommand, CodinglagomEvent, CodinglagomState](
        persistenceId = persistenceId,
        emptyState = CodinglagomState.initial,
...
/**
  ...
  */
case class CodinglagomState(message: String, timestamp: String) {
  def applyCommand(cmd: CodinglagomCommand): ReplyEffect[CodinglagomEvent, CodinglagomState] =
    cmd match {
      case x: Hello              => onHello(x)
      case x: UseGreetingMessage => onGreetingMessageUpgrade(x)
    }

  def applyEvent(evt: CodinglagomEvent): CodinglagomState =
    evt match {
      case GreetingMessageChanged(msg) => updateMessage(msg)
    }
  private def onHello(cmd: Hello): ReplyEffect[CodinglagomEvent, CodinglagomState] =
    Effect.reply(cmd.replyTo)(Greeting(s"$message, ${cmd.name}!"))

  private def onGreetingMessageUpgrade(
    cmd: UseGreetingMessage
  ): ReplyEffect[CodinglagomEvent, CodinglagomState] =
    Effect
...
object CodinglagomState {

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  def initial: CodinglagomState = CodinglagomState("Hello", LocalDateTime.now.toString)
  ...
  /**
    ...
    */
  val typeKey = EntityTypeKey[CodinglagomCommand]("CodinglagomAggregate")
  ...
  /**
    ...
    */
  implicit val format: Format[CodinglagomState] = Json.format
}

/**
  * This interface defines all the events that the CodinglagomAggregate supports.
  */
sealed trait CodinglagomEvent extends AggregateEvent[CodinglagomEvent] {
  def aggregateTag: AggregateEventTag[CodinglagomEvent] = CodinglagomEvent.Tag
}

object CodinglagomEvent {
  val Tag: AggregateEventTag[CodinglagomEvent] = AggregateEventTag[CodinglagomEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends CodinglagomEvent
...
/**
  ...
  */
trait CodinglagomCommandSerializable

/**
  * This interface defines all the commands that the CodinglagomAggregate supports.
  */
sealed trait CodinglagomCommand
    extends CodinglagomCommandSerializable
...
/**
  ...
  * application loader.
  */
object CodinglagomSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    // state and events can use play-json, but commands should use jackson because of ActorRef[T] (see application.conf)
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[CodinglagomState],
